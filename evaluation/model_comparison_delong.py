"""
Statistical Model Comparison using DeLong's Test with Multiple Comparison Correction

Performs pairwise comparisons of AUROC between CLIP, ResNet-50, and Swin models
using aggregated out-of-fold predictions with Holm-Bonferroni correction.
"""
...
def load_fold_predictions(logs_dir: Path, fold: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # Load from CSV format (expected columns: sample_id, true_label, predicted_label, predicted_probability)
...
def load_predictions_from_csv(csv_file: Path, model_name: str) -> Tuple[np.ndarray, np.ndarray]:
    """
    Alternative method: Load predictions from evaluation CSV files.
    
    Assumes CSV has columns: sample_id, fold, label, prediction_<model>
    """
    df = pd.read_csv(csv_file)
    
    # Filter for test folds and get predictions
    if 'fold_test' in df.columns and 'fold' in df.columns:
        # Get test samples (where fold == fold_test)
        test_df = df[df['fold'] == df['fold_test']].copy()
    else:
        test_df = df.copy()
...
def main():
    ...
    print("="*80)
    print("STATISTICAL MODEL COMPARISON: DeLong's Test with Holm-Bonferroni Correction")
    print("="*80)
    
    # Configuration
    logs_base = Path("logs")
    models_config = {
        "CLIP": "CLIP_ViT_Large_Baseline",
        "ResNet": "ResNet50_Baseline",
        "ConvNeXt": "ConvNeXt_Baseline"
    }
    ...
    for model_name, predictions in models_predictions.items():
        ...
        print(f"  {model_name:10s}: AUROC = {auroc:.4f}")
    ...
    if len(raw_p_values) > 1:
        ...
        print("\n  Results:")
        print("  " + "="*76)
        print(f"  {'Comparison':<20} {'ΔAUROC':<12} {'Raw p':<12} {'Adj. p':<12} {'Significant'}")
        print("  " + "-"*76)
        
        for i, (m1, m2) in enumerate(comparisons):
            status = "✓ YES" if reject[i] else "✗ NO"
            print(f"  {m1:>8} vs {m2:<8} {auc_differences[i]:>+10.4f}  "
    ...
    for rank, (model_name, auroc) in enumerate(sorted_models, 1):
        print(f"    {rank}. {model_name:10s}: {auroc:.4f}")
    ...
    # Save results
    output_file = Path("evaluation/results/model_comparison_delong.json")
